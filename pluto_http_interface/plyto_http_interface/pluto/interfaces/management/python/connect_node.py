...
class ConnectNodeView(PlutoManageAPIView):  # pylint: disable=abstract-method
    ...
    def handle_put(self, request: Request, **kwargs) -> Response:
        ...
        try:
            if request.body is None:
                return Response(
                    status=HTTPStatus.BAD_REQUEST, data={"result": False, "description": "Missing request body."}
                )
            ...
            try:
                ...
                if not serializer.is_valid():
                    return Response(
                        status=HTTPStatus.BAD_REQUEST, data={"result": False, "description": "Invalid request body."}
                    )
