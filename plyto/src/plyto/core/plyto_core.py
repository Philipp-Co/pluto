...
class PlytoCore:
    ...
    def build(self) -> Self:
        self.__logger.info("Build Configuration...")
        ...
        for node in self.__nodes:
            with open(f"{self.workdir()}{self.__nodes[node].name()}.txt", "w") as file:
                file.write(dumps(self.__nodes[node].config().to_string()))

        with open(f"{self.workdir()}core.txt", "w") as file:
            file.write(dumps(self.config().to_string()))
            file.flush()

        self.__logger.info("Build Configuration done.")
        return self
    ...
    def node_configurations(self) -> List[PlytoNodeConfig]:
        return [self.__nodes[node].config() for node in self.__nodes]
